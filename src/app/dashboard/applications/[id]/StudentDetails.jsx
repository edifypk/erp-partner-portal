...
const StudentDetails = ({ application }) => {
    return (
            <div className='bg-white border border-dashed rounded-2xl'>
                        ...
                        <div className='flex items-center gap-2'>
                            <div>
                                <Avatar className='w-9 h-9 border'>
                                    <AvatarImage className='object-cover border border-white rounded-full' src={application?.student?.contact?.photo_url || `/images/placeholder/male.png`} />
                                    <AvatarFallback>{application?.student?.contact?.name?.charAt(0)}</AvatarFallback>
                                </Avatar>
                            </div>
                            <div>
                                <div className='text-black text-xs font-medium tracking-tight whitespace-nowrap'>{application?.student?.contact?.name}</div>
                                <div className='text-[10px] font-medium tracking-tighter -translate-y-[1px] text-gray-500  whitespace-nowrap'>{application?.student?.contact?.contact_id}</div>
                            </div>
                        </div>
                        <TooltipProvider>
                ...
                <div className='border-0 border-dashed py-3 px-4 text-xs space-y-2 tracking-tight'>
                    {
                        [
                            {
                                label: "Gender",
                            ...
                            {
                                label: "Nationality",
                                value: application?.student?.contact?.nationality
                            },
                            {
                                label: "City",
                            ...
                            {
                                label: "Address",
                                value: application?.student?.lead?.address?.length > 30 ? application?.student?.lead?.address?.slice(0, 30) + '...' : application?.student?.lead?.address
                            }
                        ].map((item, index) => (
                            <div key={index} className='flex gap-2 items-center justify-between'>
                                <div className='font-medium text-gray-500'>{item.label}</div>
                                <div className={`${item.valueClass || ""} break-words`}>{item.value || '--'}</div>
                            </div>
                        ))
                    }
                </div>
