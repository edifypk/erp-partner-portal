...
const schema = z.object({
    file_name: z.string().min(1, "Name is required").max(100, "Name must be less than 100 characters"),
});


const UpdateModal = ({file,children}) => {
        ...
        const submitPromise = new Promise(async (resolve, reject) => {
            try {
                const res = await axios.put(
                    `${process.env.NEXT_PUBLIC_BACKEND_URL}/crm/student-docs-files/${file?.id}`,
                    data
                );
        ...
        const fetchUpdatedValues = async () => {
            try {
                const res = await axios.get(
                    `${process.env.NEXT_PUBLIC_BACKEND_URL}/crm/student-docs-files/file/${file?.id}`
                );
                if (res.status === 200) {
                    form.reset({
