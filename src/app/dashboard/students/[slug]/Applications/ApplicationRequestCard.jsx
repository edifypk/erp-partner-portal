...
const ApplicationRequestCard = ({ application, student }) => {
    ...
    return (
        <>
            ...
            <Link href={`/dashboard/crm/applications-requests/${application?.application_id}`} className={`hover:ring-2 border ring-blue-200 block hover:shadow-sm cursor-pointer bg-white rounded-2xl transition-all duration-300 pb-4`}>
                    ...
                    <div>
                        <div className='flex items-center justify-center'>
                            <Badge
                                variant="soft"
                                size="2"
                                color={applicationRequestsStatuses.find(status => status.slug === application?.approval_status)?.color}
                                radius='full'
                                className='capitalize'
                            >
                                {applicationRequestsStatuses.find(status => status.slug === application?.approval_status)?.name}
                            </Badge>
                        </div>
                    </div>
                </div>
