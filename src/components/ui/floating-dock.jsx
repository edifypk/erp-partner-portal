...
const FloatingDockDesktop = ({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "mx-auto hidden h-16 items-end gap-4 rounded-full bg-white/70 backdrop-blur-md px-4 pb-3 md:flex",
        className
      )}>
      {items.map((item) => (
...
function IconContainer({
  ...
  return (
    <Link href={href} target={isInternal ? "_self" : "_blank"}>
      <motion.div
        ref={ref}
        style={{ width, height }}
        onMouseEnter={() => setHovered(true)}
        onMouseLeave={() => setHovered(false)}
        className="relative flex aspect-square border border-gray-300 items-center justify-center rounded-full bg-white/90 backdrop-blur-md">
        <AnimatePresence>
          {hovered && (
            <motion.div
