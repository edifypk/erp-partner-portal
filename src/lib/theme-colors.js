...
export default function setGlobalColorTheme(themeMode, color) {
    ...
    if (!theme) {
        console.error(`Theme not found: ${color} - ${themeMode}`);
        return;
    }

    // Convert camelCase keys to kebab-case and set CSS variables
    for (const key in theme) {
        // Convert camelCase to kebab-case (e.g., cardForeground -> card-foreground)
        const cssVarName = key.replace(/([A-Z])/g, '-$1').toLowerCase();
        
        // Set the CSS variable with hsl() wrapper
        const value = `hsl(${theme[key]})`;
